import type { SiteContent } from "@/types/content";

const nl: SiteContent = {
  seo: {
    title: "Barbershop Gent | Herenkapper & Baardverzorging",
    description:
      "Professionele barbershop in Gent voor herenkapsels, baardverzorging en knippen met de tondeuse. Bezoek ons in Noordstraat 27, 9000 Gent.",
  },

  nav: {
    services: "Diensten",
    about: "Over ons",
    location: "Locatie",
    book: "Afspraak maken",
  },

  hero: {
    eyebrow: "Barbershop in Gent",
    title: "Jouw stijl, tot in de details",
    subtitle:
      "Professionele herenkapsels en baardverzorging in een moderne en ontspannen sfeer in het hart van Gent.",

    primaryButton: "Afspraak maken",
    secondaryButton: "Bekijk diensten",

    locationLabel: "Locatie",
    location: "Noordstraat 27, 9000 Gent",

    hoursLabel: "Open",
    hours: "Vanaf 10:30",

    viewMap: "Bekijk kaart",
  },

  services: {
    eyebrow: "Onze diensten",
    title: "Alles voor jouw stijl",
    description:
      "Van een frisse knipbeurt tot een verzorgde baard. Ontdek onze diensten en kies wat bij jou past.",

    items: [
      {
        title: "Haar knippen",
        description:
          "Een verzorgde knipbeurt, afgestemd op jouw haar en persoonlijke stijl.",
        price: "€20",
      },
      {
        title: "Kinderen tot 7 jaar",
        description:
          "Een comfortabele en verzorgde knipbeurt voor kinderen tot 7 jaar.",
        price: "€15",
      },
      {
        title: "Baard scheren",
        description:
          "Een gladde en verzorgde scheerbeurt voor een frisse uitstraling.",
        price: "€12",
      },
      {
        title: "Baard modelleren",
        description:
          "Je baard wordt netjes getrimd en in model gebracht voor een strakke afwerking.",
        price: "€12",
        promo: "Promo",
      },
      {
        title: "Haar + baard",
        description:
          "Een complete verzorging van haar en baard voor een perfect afgewerkte look.",
        price: "€28",
      },
      {
        title: "Haar & baard met scheermes",
        description:
          "Haar en baard volledig met het scheermes, glad en zorgvuldig afgewerkt.",
        price: "€25",
        promo: "Promo",
      },
    ],
  },

  about: {
    eyebrow: "Over ons",
    title: "Meer dan alleen een knipbeurt",
    text:
      "Bij ons draait het om vakmanschap, aandacht en een stijl die bij jou past. In een moderne en ontspannen omgeving nemen we de tijd voor een verzorgde afwerking, zodat je met een frisse look en een goed gevoel buitenstapt.",

    experience: "Jaren ervaring",
    experienceDescription:
      "Vakmanschap met aandacht voor jouw stijl.",

    quality: "Persoonlijke service",
    qualityDescription:
      "Persoonlijke aandacht en een verzorgde afwerking.",
  },

  gallery: {
    eyebrow: "Onze sfeer",
    title: "Een plek waar stijl en detail samenkomen",
  },

  location: {
    eyebrow: "Locatie",
    title: "Kom langs in Gent",
    text:
      "Je vindt onze barbershop aan de Noordstraat 27 in Gent. Kom langs voor een frisse look en persoonlijke service.",

    addressLabel: "Adres",
    address: "Noordstraat 27, 9000 Gent",

    hoursLabel: "Openingsuren",
    hours: [
      { day: "Maandag", time: "10:30 - 18:30" },
      { day: "Dinsdag", time: "Gesloten" },
      { day: "Woensdag", time: "10:30 - 18:30" },
      { day: "Donderdag", time: "10:30 - 18:30" },
      { day: "Vrijdag", time: "10:30 - 18:30" },
      { day: "Zaterdag", time: "10:30 - 18:30" },
      { day: "Zondag", time: "10:30 - 18:30" },
    ],

    directions: "Route bekijken",
    whatsapp: "Afspraak maken",
  },

  footer: {
    tagline:
      "Vakmanschap, stijl en persoonlijke service in het hart van Gent.",
    navigation: "Navigatie",
    contact: "Contact",
    services: "Diensten",
    about: "Over ons",
    location: "Locatie",
    rights: "Alle rechten voorbehouden.",
    whatsapp: "Afspraak maken via WhatsApp",
    developerCredit: "Ontworpen en ontwikkeld door",
  },
};

export default nl;